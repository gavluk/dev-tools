...
TESTS=`find -name "*.java" | grep "src/test" | wc -l`

LINES=`find -name "*.java" | xargs wc -l | awk '/total/{k+=$1}END{print k}';`
TESTS_LINES=`find -name "*.java" | grep "src/test" | xargs wc -l | awk '/total/{k+=$1}END{print k}';`

RETURN_NULL=`find -name "*.java" | xargs grep "return null" | wc -l`
